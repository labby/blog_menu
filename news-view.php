...
// -------------------------------------------------------------------------
// Adpted version for the Blog-menu snippet.
// Original adapted by Eki, changes merged for WB2.7 by Ruud
// -------------------------------------------------------------------------
/*
	...
	if($setting_posts_per_page != 0) {
		if($position > 0) {
			if(isset($_GET['g']) AND is_numeric($_GET['g'])) {
				...
				$pl_prepend = '<a href="?p='.($position-$setting_posts_per_page).'&y='.$_GET['y'].'&m='.$_GET['m'].'&method='.$_GET['method'].'"><< ';
			} else {
		...
		if($position+$setting_posts_per_page >= $total_num) {
			...
			if(isset($_GET['g']) AND is_numeric($_GET['g'])) {
				...
				$nl_prepend = '<a href="?p='.($position+$setting_posts_per_page).'&y='.$_GET['y'].'&m='.$_GET['m'].'&method='.$_GET['method'].'"> ';
			} else {
	...
	if($num_posts > 0) {
		if($query_extra != '') {
			...
			<div class="selected_group_title">
				<?php if(isset($_GET['g']) AND is_numeric($_GET['g'])) {
					echo '<a href="'.htmlspecialchars(strip_tags($_SERVER['PHP_SELF'])).'">'.PAGE_TITLE.'</a> >> '.$groups[$_GET['g']]['title'];
				}elseif(isset($_GET['m']) AND is_numeric($_GET['m']) AND isset($_GET['y']) AND is_numeric($_GET['y'])) {
					$month = date("F Y", mktime(0, 0, 0, $_GET['m'], 1, $_GET['y']));
					echo '<a href="'.htmlspecialchars(strip_tags($_SERVER['PHP_SELF'])).'">'.PAGE_TITLE.'</a> >> '.$month;
				} ?>
			</div>
			<?php
		}
